...
| [Fourmilab Analytical Engine](https://fourmilab.ch/babbage/contents.html) | Analytical Engine | Reconstruction/emulator lineage + documentation | Unified textual Number/Variable/Operation card stream (an explicit emulator simplification) | Yes: Step processes current card, advances, and halts | Annunciator shows Mill axes/operation, current card, and populated Store columns | Trace is available in Attendant log; not the repository's event schema | Reuse terms/code only after checking site terms; link/cite is preferred | Not responsibly verified | Card execution and whole-emulator behavior; do not recast its syntax as primary evidence | Evidence-separated historical roles plus compact tested intermediate-value flow |
