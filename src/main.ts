...
function visibleCarry() {
  ...
  const carryProfiles = CARRY_EVIDENCE_PROFILES.map(profile => `<details><summary><b>${copy(profile.family)}</b> · ${profile.dateOrModel} · ${profile.claimType}/${profile.evidenceStrength}</summary><p><a href="${profile.sourceUrl}" target="_blank" rel="noreferrer">${esc(profile.sourceLabel)}</a></p><b>${t('Documented carry responsibility', '资料支持的进位责任')}</b><ul>${profile.documentedRoles.map(item => `<li>${copy(item)}</li>`).join('')}</ul><p><b>${t('Operator implication:', '对操作者意味着：')}</b> ${copy(profile.operatorImplication)}</p><b>${t('Not established', '未确认')}</b><ul>${profile.notEstablished.map(item => `<li>${copy(item)}</li>`).join('')}</ul></details>`).join('');
  shell(
