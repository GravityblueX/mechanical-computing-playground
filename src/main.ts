...
function sourceAtlas() {
  ...
  const group = (track: SourceAtlasTrack) => `<section><h2>${copy(trackLabel[track])}</h2><div class="structure-callout"><b>${t('Generation and source identity remain explicit', '机器世代与来源身份保持明确')}</b><p>${track === 'difference-engine-no-2' ? t('Babbage archive designs/notations are H/E1 records; the 1991/2002 built engine is an R/E2 institutional reconstruction.', 'Babbage 档案中的设计/记法是 H/E1 记录；1991/2002 建成机器是 R/E2 机构复原。') : t('Surviving component roles do not prove that the pieces were wired in the exact browser order.', '存世部件的功能并不能证明它们按浏览器中的确切顺序连接。')}</p></div>${sourceAnchorsForTrack(track).map(anchor => `<article class="structure-card"><h3>${esc(anchor.sourceTitle)}</h3><p><b>${anchor.claimType}/${anchor.evidenceStrength}</b> · ${esc(anchor.accessKind)} · ${esc(anchor.institution)}</p><p><b>${t('Record:', '记录：')}</b> <code>${esc(anchor.recordIdentifier)}</code><br><b>${t('Generation:', '世代：')}</b> ${copy(anchor.generation)}<br><b>${t('Inspected:', '检查日期：')}</b> ${anchor.inspectedDate}</p><p><a href="${anchor.sourceUrl}" target="_blank" rel="noreferrer">${t('Open institutional source', '打开机构来源')}</a> · <code>${esc(anchor.researchNoteAnchor)}</code></p><b>${t('This source supports', '本来源支持')}</b><ul>${anchor.supports.map(item => `<li>${copy(item)}</li>`).join('')}</ul><details><summary>${t('It does not establish', '它不能确认')}</summary><ul>${anchor.notEstablished.map(item => `<li>${copy(item)}</li>`).join('')}</ul></details>${anchor.fullFacsimileInspected === false ? `<p class="model-note"><b>${t('Bibliographic-only boundary:', '仅书目边界：')}</b> ${t('no full facsimile was inspected, so there are no page/figure claims.', '未检查完整影印本，因此没有页码/图号级主张。')}</p>` : ''}</article>`).join('')}</section>`;
  shell(
    ...
    `<section>${lesson({ en: 'Trace each historical role to one inspected record.', zh: '把每个历史功能追溯到一条已检查记录。' }, { en: 'Read “supports” before opening “does not establish.”', zh: '先读“本来源支持”，再展开“不能确认”。' }, { en: 'An archive image is not self-interpreting proof of every physical detail.', zh: '档案图像不会自行证明每一个物理细节。' })}<div class="structure-callout"><b>${t('Evidence adapter—not geometry reconstruction', '证据适配层——不是几何复原')}</b><p>${t('No card infers tooth counts, linkage, timing, force, tolerance, performance, or full-machine wiring from appearance alone.', '没有资料卡仅凭外观推断齿数、连杆、时序、力、公差、性能或整机连接。')}</p><p><a href="#/finite-difference">${t('Difference/output lesson', '差分/输出课程')}</a> · <a href="#/continuous">${t('Continuous lesson', '连续计算课程')}</a></p></div></section>${group('difference-engine-no-2')}${group('bush-differential-analyzer')}`
  );
}
