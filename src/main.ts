...
function lesson(why: Copy, watch: Copy, learn: Copy) {
  return `<div class="lesson"><div><span>① ${t('WHY PLAY?', '为什么玩？')}</span><p>${copy(why)}</p></div><div><span>② ${t('WHAT TO WATCH', '看什么？')}</span><p>${copy(watch)}</p></div><div><span>③ ${t('TAKEAWAY', '学到什么？')}</span><p>${copy(learn)}</p></div></div>`;
}
...
function overview() {
  shell(
    ...
    <section><h2>${t('You already know enough to begin', '你已经具备开始所需的全部知识')}</h2><p class="plain">${t('If you can use +, −, ×, ÷ on a calculator, you can use this museum. No history, engineering, or calculus is required.', '只要会在计算器上按 ＋、－、×、÷，就能玩这个展厅。不需要懂机械史、工程学或微积分。')}</p><div class="journey"><a href="#/visible-carry"><b>1</b><span>99 + 1</span><small>${t('Why does 9 become 0?', '为什么 9 会变成 0？')}</small></a><a href="#/finite-difference"><b>2</b><span>0, 1, 4, 9…</span><small>${t('Make squares using only +', '只用加法生成平方数')}</small></a><a href="#/multiplication"><b>3</b><span>314 × 27</span><small>${t('Turn × into repeated motions', '把乘法拆成重复动作')}</small></a><a href="#/hand-crank-backprop"><b>4</b><span>${t('A machine learns', '机器学习')}</span><small>${t('See an error travel backward', '看误差怎样倒着传回去')}</small></a></div></section>
...
function visibleCarry() {
  ...
  shell(
    ...
    `${lesson({ en: 'Carry is the hidden difficulty inside ordinary addition.', zh: '“进位”是普通加法里最容易被忽略、却最难机械实现的部分。' }, { en: 'Press “one movement” and follow the highlighted wheel and ← arrows.', zh: '点击“执行一个动作”，跟着发亮的数字轮和 ← 箭头看。' }, { en: '100 is not one action: two 9s roll to 0, then the hundreds wheel moves to 1.', zh: '100 不是一个动作产生的：两个 9 先转成 0，百位轮再转成 1。' })}<section><div class="equation"><span>0099</span><b>+ 1</b><strong>= ${digitsToString(carryState().digits)}</strong></div>${carryDiagram(digits)}<div class="controls"><button id="carry-step">${t('Do one movement', '执行一个动作')}</button><button id="carry-crank">${t('Show the whole addition', '演示完整加法')}</button><button class="secondary" id="carry-reset">${t('Start again', '重新开始')}</button></div><div class="progress"><i style="width:${carryIndex / carryTrace.events.length * 100}%"></i></div><p class="status">${t('Movement', '动作')} ${carryIndex} / ${carryTrace.events.length}</p><details open><summary>${t('Movement log in plain language', '用白话记录每个动作')}</summary><pre>${esc(eventText())}</pre></details></section>`
  );
...
function finiteDifference() {
  ...
  shell(
    ...
    `${lesson({ en: 'It explains why a Difference Engine can print mathematical tables without a general multiplier.', zh: '它解释了差分机为什么不需要通用乘法器，也能印出数学表。' }, { en: 'Watch Δ² stay at 2 while Δ¹ and the output grow.', zh: '观察 Δ² 一直等于 2，而 Δ¹ 和输出数字不断增长。' }, { en: 'A constant higher difference lets repeated addition generate a polynomial sequence.', zh: '高阶差分保持不变时，只靠重复加法就能生成多项式数列。' })}<section><div class="formula-story"><div><small>${t('numbers we want', '想要的数字')}</small><b>0, 1, 4, 9, 16…</b></div><span>↓ ${t('difference', '相邻相减')}</span><div><small>${t('first differences', '一阶差分')}</small><b>1, 3, 5, 7…</b></div><span>↓ ${t('difference again', '再相减')}</span><div class="constant"><small>${t('second difference', '二阶差分')}</small><b>2, 2, 2, 2…</b></div></div><div class="bar-chart">${bars}</div><div class="state-grid">${diff.columns.map((value, order) => `<div><small>${order === 0 ? t('output now', '当前输出') : `Δ${order}`}</small><strong>${value}</strong><span>${order === diff.columns.length - 1 ? t('stays constant', '保持不变') : t('receives the number on its right', '接收右边的数字')}</span></div>`).join('')}</div><p><b>${t('Numbers produced:', '已经生成：')}</b> ${diff.output.join(' → ')}</p><p><b>${t('This crank did:', '这一轮执行：')}</b> ${diffEvents.join('，') || t('nothing yet', '尚未转动')}</p><div class="controls"><button id="diff-step">↻ ${t('Turn crank once', '转动曲柄一次')}</button><button class="secondary" id="diff-square">${t('Square-number preset', '平方数预设')}</button><button class="secondary" id="diff-cubic">${t('Cube-number preset', '立方数预设')}</button></div></section>`
  );
...
function multiplication() {
  ...
  shell(
    ...
    `${lesson({ en: 'The × key hides a sequence of smaller operations.', zh: '计算器上的 × 键，把一连串更小的动作藏起来了。' }, { en: 'Compare how 7 ones and 2 tens are represented in three mechanisms.', zh: '比较三种机构怎样分别处理 7 个“一”和 2 个“十”。' }, { en: 'The same algorithm can be embodied by different physical parts.', zh: '同一种算法，可以由不同的机械结构来表达。' })}<section><div class="equation"><span>314 × 27</span><strong>= ${result.value}</strong></div><div class="place-decomposition"><div>27</div><span>=</span><b>7 × 1</b><span>+</span><b>2 × 10</b></div><div class="mechanism-lanes"><div><h3>${t('Repeated addition', '重复加法')}</h3><div class="motion">${Array.from({ length: 7 }, () => '<i>+</i>').join('')}<em>… ×27</em></div><p>${t('Add 314 twenty-seven times.', '把 314 连加 27 次。')}</p></div><div><h3>${t('Stepped drum', '阶梯鼓轮')}</h3><div class="drum" aria-label="stepped drum">▂▄▆█ <b>↻</b></div><p>${t('A selected depth exposes a chosen number of steps.', '用选择的深度决定有多少级台阶参与啮合。')}</p></div><div><h3>${t('Pinwheel', '拨轮')}</h3><div class="pinwheel" aria-label="pinwheel">${Array.from({ length: 10 }, (_, i) => `<i class="${i < 7 ? 'on' : ''}">•</i>`).join('')}</div><p>${t('Expose 7 pins, then shift the carriage for 2 tens.', '先露出 7 根销齿，再移动位架处理 2 个十。')}</p></div></div><div class="shift-demo"><span>314 × 7 = 2198</span><b>+</b><span class="shifted">314 × 2 × <mark>10</mark> = 6280</span><b>= 8478</b></div><p class="model-note">${t('These diagrams explain operation, not exact historical geometry.', '这些图解释的是操作逻辑，不宣称精确还原历史机器的几何结构。')}</p></section>`
  );
}
